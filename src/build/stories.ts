...
function evalValue(value: string): StoryMeta | undefined {
  try {
    return new Function(`return (${value})`)() as StoryMeta; // oxlint-disable-line
  } catch {
    return;
  }
}
...
function filterOutDucktoryTags(content: string): string {
  const ducktoryDocumentationRegex =
    /<ducktory-?documentation[^>]*>[\s\S]*?<\/ducktory-?documentation>/gi;
